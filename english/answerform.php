...
" class="big" 
	    ...
	    <p><input type="button" id="portal" value="back" onClick="location.href='question.php'" style="width:100%;font-size:50px"></p>
	    ...
	    $con = mysql_connect('localhost','account','password','service'); 
	    $db = mysql_select_db('english',$con);
	    $sql = "select * from user_qlist where qID=".$qid." and username=\"{$_SESSION['name']}\" order by answerdate desc limit 1";
	    $result = mysql_query($sql);
	    $row = mysql_fetch_assoc($result);
	    $ans=$row['useranswer'];
	    mysql_close($con);
	    print "<h1>Answer {$_SESSION['qname']} / {$_SESSION['name']}</h1>\n";
	    print "<form id=\"qtable\"><table width=\"100%\" style=\"font-size:50px\">";
	    for ($i=0;$i<200;$i++){
